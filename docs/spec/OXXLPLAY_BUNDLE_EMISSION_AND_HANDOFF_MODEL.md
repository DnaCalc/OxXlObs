# OXXLPLAY_BUNDLE_EMISSION_AND_HANDOFF_MODEL.md

## 1. Position
This document defines how OxXlPlay should turn retained observations into replay-ready evidence for `OxReplay`.
...
1. `W006` retains a live replay-ready bundle under `states/excel/xlplay_capture_values_formulae_001/bundle.json`.
...
## 7. Downstream consumer labeling rule
1. The normalized replay view is a lossy projection. Downstream consumers must label comparison results derived from it as `lossy` rather than as direct semantic equivalence.
2. The richer observation bundle and its provenance sidecars remain the authoritative observation evidence and must not be discarded in favor of the normalized view alone.
3. The full downstream labeling and interpretation contract for `DNA OneCalc` is in `docs/spec/OXXLPLAY_ONECALC_OBSERVATION_CONSUMER_CONTRACT.md`.

## 8. W007 canonical replay-manifest alignment
1. `W007` begins aligning `OxXlPlay` emission with the `OxReplay` canonical bundle manifest shape.
2. The current alignment model is dual-artifact:
   - the rich `OxXlPlay` observation bundle remains the source observation contract,
