...
use oxxlplay_capture::{CaptureValidationError, ObservationCapture, validate_capture};
use oxxlplay_provenance::{ObservationProvenance, ProvenanceValidationError, validate_provenance};
use oxxlplay_scenario::{ObservationScenario, ScenarioValidationError, validate_scenario};
...
mod tests {
    ...
    const WITNESS_FIXTURE: &str = include_str!(
        "../../../docs/test-corpus/bundles/xlplay_witness_seed_divergence_001/witness-seed.json"
    );
    const LIVE_DRIVER_BUNDLE_FIXTURE: &str =
        include_str!("../../../states/excel/xlplay_capture_values_formulae_001/bundle.json");
    ...
    fn assembles_bundle_seed_from_valid_parts() {
        ...
        assert_eq!(assembled.bundle_schema, "oxxlplay.replay_bundle_seed.v1");
        assert_eq!(
